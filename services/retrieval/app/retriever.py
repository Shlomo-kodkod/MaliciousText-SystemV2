...
from services.retrieval.app import config 
...
class Retrieval:
    def __init__(self):
        self.__dal = DAL(config.protocol, config.username, config.password, config.cluster, config.db)
    ...
    def __publish_tweet(self, tweet: list, label: str = "Antisemitic"):
        try:
            for record in tweet:
