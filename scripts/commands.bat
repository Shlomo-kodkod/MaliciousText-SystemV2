...
docker build -f services/preprocessor/Dockerfile -t kodkod1docker/preprocessor:v1.0 .
...
docker run -d  --name broker --network text-analyzer -e KAFKA_NODE_ID=1 
...
docker run -d --name mongodb-community-server --network text-analyzer mongodb/mongodb-community-server:latest


docker run --name retriever -d --network text-analyzer kodkod1docker/retriever:v1.0

docker run --name preprocessor -d --network text-analyzer kodkod1docker/preprocessor:v1.0
